...
// // Modificación en `performAttack` para verificar ganador
// function performAttack(type) {
//     if (isGameOver || !selectedCharacter || !selectedEnemy) return;
...
//     switch (type) {
//         case 'normal':
//             damage = Math.floor(Math.random() * 10) + 5; // Daño entre 5 y 15
//             moveMessage = '¡Estocada!';
//             break;
//         case 'special':
//             if (playerEnergy >= 30) {
//                 damage = Math.floor(Math.random() * 15) + 15; // Daño entre 15 y 30
//                 playerEnergy -= 30;
//                 moveMessage = '¡Golpe de Alma!';
//             } else {
//                 showMessage('¡No tienes suficiente energía!');
...
//         case 'ultra':
//             if (playerEnergy >= 100) {
//                 damage = Math.floor(Math.random() * 25) + 35; // Daño entre 35 y 60
//                 playerEnergy = 0;
//                 moveMessage = '¡IRA DE LOS DIOSES!';
//             } else {
//                 showMessage('¡Necesitas energía máxima!');
...
//     if (combo > 0) {
//         damage = Math.floor(damage * (1 + combo * 0.1)); // Incremento por combo
//         energyGain += combo * 5;
//     }
...
function restart() {
    ...
    document.querySelectorAll('.end-screen').forEach(el => el.remove());
...
// Restablecer el juego al reiniciar
function restart() {
    playerHealth = 100;
    enemyHealth = 100;
    playerEnergy = 0;
...
// Modificar la función `performAttack` para llamar a `gameOver` correctamente
function performAttack(type) {
    if (isGameOver || !selectedCharacter || !selectedEnemy) return;
...
// Modificar la función `enemyTurn` para llamar a `gameOver` correctamente
function enemyTurn() {
    if (isGameOver) return;
